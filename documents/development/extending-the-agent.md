...
| --- | --- | --- |
| Orchestration or researcher behavior | `research_agent/prompts.py`, assembled in `agent.py` | `tests/test_prompts_validation.py`, relevant agent contract tests |
| Agent tool | `research_agent/tools.py`, registered in `agent.py` | focused tool tests plus agent contracts when registration changes |
| Chat or embedding provider | `model_factory.py` | provider branch tests and `tests/test_retry_utils.py` when retry wrapping changes |
| Output skill | `.deepagents/skills/<skill>/SKILL.md` | `tests/test_skill_registry.py`, `tests/test_skill_contracts.py` |
| Delegation topology or limits | `agent.py`, `research_agent/prompts.py` | prompt validation and `tests/test_agent_contracts.py` |
| Document format or context limit | `research_agent/utils/content_extractors.py`, `research_agent/utils/knowledge_filesystem.py` | `tests/test_tools.py` plus upload or ingestion tests when those surfaces change |
...
| --- | --- |
| `RESEARCH_WORKFLOW_INSTRUCTIONS` | Orchestrator workflow, planning, synthesis, citations, and report-writing rules; included in `agent.py`'s `INSTRUCTIONS`. |
| `SUBAGENT_DELEGATION_INSTRUCTIONS` | Orchestrator delegation strategy and formatted concurrency/iteration limits; appended to `INSTRUCTIONS` in `agent.py`. |
...
Keep these constants in `research_agent/prompts.py`. `research_agent/__init__.py` re-exports the three instruction sets used by the orchestrator; update that public export only when adding or renaming a public constant.

For each prompt change:

1. add or update an assertion in `tests/test_prompts_validation.py` before changing the instruction;
2. update `agent.py` only when assembly, formatting inputs, or consumers change;
...
Define the LangChain `@tool` wrapper in `research_agent/tools.py`. Put substantial I/O or reusable logic in a focused module under `research_agent/utils/`, keep state-only parameters injected with `InjectedState` or `InjectedToolArg`, and document purpose, when to call the tool, arguments, return value, and failure behavior in its docstring.

Register the tool with the agent that needs it:

- add orchestrator tools to `_agent_kwargs["tools"]` in `agent.py` when they need document state, local files, wiki access, or report writes;
- add narrowly scoped web-research tools to `research_sub_agent["tools"]` when isolated sub-agents should call them;
- export a stable public tool from `research_agent/__init__.py` only when callers outside `agent.py` need it;
...
- extend authentication helpers in `model_factory.py` when the provider supports multiple credential modes;
...
`agent.py` passes all three roots to Deep Agents in the order shown. `research_agent/utils/skill_registry.py` scans the built-in root before the custom root, while `webapp/routes.py` installs uploaded skills into `docs/.deepagents/skills/`.
...
For strategy-only changes, edit `SUBAGENT_DELEGATION_INSTRUCTIONS` and its prompt assertions. For capability or topology changes, update `research_sub_agent`, `_agent_kwargs["subagents"]`, and the corresponding tool registrations in `agent.py`; keep `MAX_CONCURRENT_RESEARCH_UNITS` and `MAX_RESEARCHER_ITERATIONS` aligned with the formatted prompt and configuration docs.
...
`ResearchStateMiddleware` in `agent.py` seeds `/research_request.md`, injects current files into state context, configures the document folder, and leaves uploaded-file access with the orchestrator. `read_docs_folder` in `research_agent/tools.py` delegates extraction to `read_docs_folder_impl` in `research_agent/utils/knowledge_filesystem.py`, which constrains reads to the configured folder, prefers a ready thread wiki, caches extracts under the active output folder's `extracted/` directory, and avoids returning oversized bodies inline.

Current direct extraction supports `.pdf`, `.txt`, `.md`, `.docx`, `.pptx`, and `.xlsx`. To add a format:

1. implement its extractor and dispatch in `research_agent/utils/content_extractors.py`;
2. add the suffix to `SUPPORTED_DOC_SUFFIXES` in `research_agent/utils/knowledge_filesystem.py`;
