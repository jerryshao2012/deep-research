...
Placeholder sources include canonical `localhost` and all its subdomains; canonical `example.com`, `example.org`, and `example.net` and all their subdomains; hostnames ending in `.example`, `.invalid`, or `.test`; ambiguous numeric-looking hosts; and literal IP addresses that are not globally routable or are loopback, private, link-local, multicast, reserved, or unspecified. Host matching is lowercase after canonicalization and ignores terminal dots.

Structural correction budget is one when optional verification is disabled or configured for zero rounds; failure occurs on next invalid check. With enabled positive `MAX_VERIFICATION_ROUNDS=N`, budget is `N` corrections and failure occurs on invalid check `N+1`. Client progress says `Citation correction 1/1 requested`; structural acceptance is never labeled as verified. Exhaustion records bounded safe defect metadata. Middleware raises `ReportCitationError` only when a configured durable checkpointer can read the exact matching failure checkpoint and its prior task writes within the bounded confirmation deadline; missing confirmation leaves terminal state blocked without claiming checkpoint-before-error, and persistence failures propagate instead. CLI rejects current returned failure or malformed report before fallback, title, output selection, or save. Web-capable verbose CLI output requires a nonempty valid report fingerprint matching its acceptance marker, withholds raw message/file content until acceptance, uses fixed pre-acceptance progress text without model-controlled metadata, and renders only current accepted final content. Raw LangGraph API transports such as `messages-tuple` remain application-consumer responsibility. Citation-containing exception groups expose only fixed categories, with persistence prioritized over citation. Error text contains no report prose, sibling exception messages, or URLs.
