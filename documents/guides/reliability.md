...
### Fail closed on unresolved web citations
...
Final failure state contains only run ID, report fingerprint, bounded safe defect categories, and terminal control metadata. With a configured durable checkpointer, middleware raises `ReportCitationError` only after the exact checkpoint values and matching prior task writes are readable from that saver. Confirmation polls with monotonic backoff for `CITATION_CHECKPOINT_CONFIRM_TIMEOUT_SECONDS` (default 5 seconds, maximum 30); invalid, non-finite, and non-positive settings use the default. Blocking reads use a fixed daemon worker/queue bound, asynchronous reads use process-wide admission held until work truly ends, and forked children create fresh bounds. Missing or not-yet-readable persistence does not make middleware claim checkpoint durability, while saver write/read errors propagate unchanged.

For web-capable or unknown-mode CLI runs, verbose display suppresses raw AI, tool-output, and file content until current report fingerprint has structural acceptance. Acceptance requires a nonempty valid fingerprint equal to the stored marker; missing or malformed fingerprints never pass through `None == None`. Before acceptance, progress uses fixed trusted phrases and never prints model-supplied tool names, roles, arguments, previews, or diagnostics. CLI then renders only accepted final content once; rejected generations are discarded. Explicit `--no-web` runs retain live message display. LangGraph's lower-level API transports, especially raw `messages-tuple` streams, are outside CLI rendering control and may carry pre-middleware model events, so direct API consumers must apply their own acceptance boundary before presenting them. A current returned failure or malformed report is rejected before retry, title generation, output selection, or save. Citation-containing exception groups are replaced with fixed safe categories; persistence remains primary over citation, but sibling messages are never re-rendered. Failure does not record evaluation metrics or expose report prose or source URLs in rendered errors. Correct report and start an explicit new run; stale failure metadata is cleared before current report is audited again.
